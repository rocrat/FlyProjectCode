...
#Then enter the spp used as a reference
print "Please enter the spp you will use as a reference:\n";
chomp(my $refspp = <STDIN>);#get the reference spp
my @allfiles = `ls /xdisk/rlapoint | grep 'PEP\.fasta'`;
my $reffile;

foreach my $file (@allfiles){#get the file id for the reference spp
	if($file =~ /$refspp/){
		chomp ( $reffile = $file);
	}
}
...
foreach my $file (@allfiles){
	...
	if($file !~ /$refspp/){
		...
		print NEWSub "time phmmer --tblout /xdisk/rlapoint/$hmmrid.hmmr1 -o /dev/null --noali $file $reffile\n";
